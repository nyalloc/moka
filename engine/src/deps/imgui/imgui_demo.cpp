...
#pragma clang diagnostic ignored \
    "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX name for this item.. // for strdup used in demo code (so user can copy & paste the code)
#pragma clang diagnostic ignored \
...
#pragma clang diagnostic ignored \
    "-Wunused-macros" // warning : warning: macro is not used // we define snprintf/vsnprintf on Windows so they are available, but not always used.
...
#if (__GNUC__ >= 6)
#pragma GCC diagnostic ignored \
    "-Wmisleading-indentation" // warning: this 'if' clause does not guard this statement      // GCC 6.0+ only. See #883 on GitHub.
#endif
#endif
    ...
    if (ImGui::CollapsingHeader("Configuration"))
    {
        ...
        if (ImGui::TreeNode("Configuration##2"))
        {
            ...
            ShowHelpMarker(
                ...
                "ImGuiConfigFlags_NavEnableSetMousePos.");
            ImGui::CheckboxFlags(
                "io.ConfigFlags: NoMouse", (unsigned int*)&io.ConfigFlags, ImGuiConfigFlags_NoMouse);
        ...
        if (ImGui::TreeNode("Backend Flags"))
        {
            ...
            ImGui::CheckboxFlags(
                "io.BackendFlags: HasGamepad", (unsigned int*)&backend_flags, ImGuiBackendFlags_HasGamepad);
            ImGui::CheckboxFlags(
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        {
            // List box
            const char* listbox_items[] = {"Apple",
                                           "Banana",
                                           "Cherry",
                                           "Kiwi",
                                           "Mango",
        ...
        if (ImGui::TreeNode("Advanced, with Selectable nodes"))
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                else
                {
                    ...
                    ImGui::TreeNodeEx(
                        (void*)(intptr_t)i, node_flags, "Selectable Leaf %d", i);
                    if (ImGui::IsItemClicked())
                        node_clicked = i;
                }
            }
    ...
    if (ImGui::TreeNode("Text"))
    {
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            ImGui::Text(
                ...
                "(kakikukeko)"); // Normally we would use u8"blah blah" with the proper characters directly in the string.
            ImGui::Text(
    ...
    if (ImGui::TreeNode("Images"))
    {
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            ImVec2 uv1 = ImVec2(
                (region_x + region_sz) / my_tex_w, (region_y + region_sz) / my_tex_h);
            ImGui::Image(
                my_tex_id,
    ...
    if (ImGui::TreeNode("Combo"))
    {
        ...
        ImGui::CheckboxFlags(
            "ImGuiComboFlags_PopupAlignLeft", (unsigned int*)&flags, ImGuiComboFlags_PopupAlignLeft);
        ...
        ImGui::Combo(
            "combo 4 (function)", &item_current_4, &FuncHolder::ItemGetter, items, IM_ARRAYSIZE(items));
    ...
    if (ImGui::TreeNode("Multi-line Text Input"))
    {
        ...
        ImGui::InputTextMultiline(
            "##source", text, IM_ARRAYSIZE(text), ImVec2(-1.0f, ImGui::GetTextLineHeight() * 16), flags);
        ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Plots Widgets"))
    {
        ...
        ImGui::PlotLines(
            "Lines", values, IM_ARRAYSIZE(values), values_offset, "avg 0.0", -1.0f, 1.0f, ImVec2(0, 80));
        ImGui::PlotHistogram(
            "Histogram", arr, IM_ARRAYSIZE(arr), 0, NULL, 0.0f, 1.0f, ImVec2(0, 80));
        ...
        ImGui::PlotLines(
            "Lines", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0, 80));
        ImGui::PlotHistogram(
    ...
    if (ImGui::TreeNode("Color/Picker Widgets"))
    {
        ...
        ImGui::ColorPicker4(
            "MyColor##4", (float*)&color, flags, ref_color ? &ref_color_v.x : NULL);
...
// Note that the SliderScalar function has a maximum usable range of half the natural type maximum, hence the /2 below.
        ...
        ImGui::InputScalar(
            ...
            ImGuiInputTextFlags_CharsHexadecimal);
        ImGui::InputScalar(
            "input s64", ImGuiDataType_S64, &s64_v, inputs_step ? &s64_one : NULL);
        ImGui::InputScalar(
            "input u64", ImGuiDataType_U64, &u64_v, inputs_step ? &u64_one : NULL);
        ImGui::InputScalar(
    ...
    if (ImGui::TreeNode("Vertical Sliders"))
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            ImGui::PushStyleColor(
                ImGuiCol_SliderGrab, (ImVec4)ImColor::HSV(i / 7.0f, 0.9f, 0.9f));
            ImGui::VSliderFloat(
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Child windows"))
    {
        ...
        // Child 1: no border, enable horizontal scrollbar
        {
            ...
            ImGui::BeginChild(
                "Child1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 260), false, window_flags);
            for (int i = 0; i < 100; i++)
            {
    ...
    if (ImGui::TreeNode("Tabs"))
    {
        ...
        if (ImGui::TreeNode("Advanced & Close Button"))
        {
            ...
            ImGui::CheckboxFlags(
                "ImGuiTabBarFlags_Reorderable", (unsigned int*)&tab_bar_flags, ImGuiTabBarFlags_Reorderable);
            ImGui::CheckboxFlags(
    ...
    if (ImGui::TreeNode("Groups"))
    {
        ...
        ImGui::Button(
            "ACTION", ImVec2((size.x - ImGui::GetStyle().ItemSpacing.x) * 0.5f, size.y));
        ImGui::SameLine();
        ImGui::Button(
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGui::BeginGroup();
            ImGui::Text(
                "%s", i == 0 ? "Top" : i == 1 ? "25%" : i == 2 ? "Center" : i == 3 ? "75%" : "Bottom");
            ImGui::BeginChild(
    ...
    if (ImGui::TreeNode("Horizontal Scrolling"))
    {
        ...
        for (int line = 0; line < lines; line++)
        {
            ...
            for (int n = 0; n < num_buttons; n++)
            {
                ...
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonActive, (ImVec4)ImColor::HSV(hue, 0.8f, 0.8f));
    ...
    if (ImGui::TreeNode("Clipping"))
    {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(
            pos, ImVec2(pos.x + size.x, pos.y + size.y), IM_COL32(90, 90, 120, 255));
...
static void ShowDemoWindowColumns()
{
    ...
    */
    ...
    if (ImGui::TreeNode("Horizontal Scrolling"))
    {
        ...
        ImGui::BeginChild(
            "##ScrollingRegion", ImVec2(0, ImGui::GetFontSize() * 20), false, ImGuiWindowFlags_HorizontalScrollbar);
        ImGui::Columns(10);
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        ...
        if (ImGui::BeginTabItem("Colors"))
        {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++)
            {
                ...
                ImGui::PushID(i);
                ImGui::ColorEdit4(
                    "##color", (float*)&style.Colors[i], ImGuiColorEditFlags_AlphaBar | alpha_flags);
        ...
        if (ImGui::BeginTabItem("Fonts"))
        {
                ...
                if (font_details_opened)
                {
                    ...
                    if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                    {
                        ...
                        for (int base = 0; base < 0x10000; base += 256)
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                                count +=
                                    font->FindGlyphNoFallback((ImWchar)(base + n)) ? 1 : 0;
                            if (count > 0 && ImGui::TreeNode(
                                                 (void*)(intptr_t)base,
                                                 "U+%04X..U+%04X (%d %s)",
                                                 base,
                                                 base + 255,
            ...
            ImGui::DragFloat(
                "global scale", &io.FontGlobalScale, 0.005f, 0.3f, 2.0f, "%.2f"); // scale everything
            ImGui::PopItemWidth();
        ...
        if (ImGui::BeginTabItem("Rendering"))
        {
            ...
            ImGui::DragFloat(
                "Global Alpha", &style.Alpha, 0.005f, 0.20f, 1.0f, "%.2f"); // Not exposing zero here so user doesn't "lose" the UI (zero alpha clips all widgets). But application code could have a toggle to switch between zero and non-zero.
            ImGui::PopItemWidth();
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::ColorEdit4("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            x += sz + spacing; // Circle
            draw_list->AddRect(
                ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f, ImDrawCornerFlags_All, th);
            x += sz + spacing;
            draw_list->AddRect(
                ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ImDrawCornerFlags_All, th);
            x += sz + spacing;
            draw_list->AddRect(
                ImVec2(x, y),
                ImVec2(x + sz, y + sz),
        ...
        draw_list->AddRectFilled(
            ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotRight);
        x += sz + spacing;
        draw_list->AddTriangleFilled(
            ImVec2(x + sz * 0.5f, y), ImVec2(x + sz, y + sz - 0.5f), ImVec2(x, y + sz - 0.5f), col32);
        x += sz + spacing;
    ...
    ImGui::Separator();
    {
        ...
        for (int i = 0; i < points.Size - 1; i += 2)
            draw_list->AddLine(
                ...
                ImVec2(
                    canvas_pos.x + points[i + 1].x, canvas_pos.y + points[i + 1].y),
                IM_COL32(255, 255, 0, 255),
                2.0f);
        draw_list->PopClipRect();
        if (adding_preview)
