...
namespace moka
{
    ...
    class graphics_device
    {
		...
		std::thread worker_; //!< Worker thread owns the graphics_api_ backend and allows the main thread to interact through messages.
		std::mutex mutex_;

		blocking_queue<std::unique_ptr<message>> messages_; //!< Visitor-based messaging system. Worker thread waits until message is enqueued.
	...
	public:
		...
		texture make_texture(void* data,
