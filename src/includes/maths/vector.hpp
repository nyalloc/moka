...
namespace loki
{
    ...
    template<typename T, size_t size, bool is_colour = false, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
    class vector
    {
    ...
    public:
        ...
        template<typename = std::enable_if_t<is_colour>>
        constexpr float r() const noexcept
        {
            return to_float(components[0]);
        }

        template<typename = std::enable_if_t<size >= 2 && is_colour>>
        constexpr float g() const noexcept
        {
            return to_float(components[1]);
        }

        template<typename = std::enable_if_t<size >= 3 && is_colour>>
        constexpr float b() const noexcept
        {
            return to_float(components[2]);
        }

        template<typename = std::enable_if_t<size >= 4 && is_colour>>
