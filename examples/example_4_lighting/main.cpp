...
struct material_uniforms
{
	...
	texture_uniform normals;
	texture_uniform roughness;
...
class lighting_application : public application
{
	...
	const char* cube_fragment_ =
		...
		void main()
		{
			...
			vec3 specular = light.specular * spec * texture(material.roughness, TexCoords).ggg;  
...
public:
	...
	lighting_application(const int argc, char* argv[])
		...
		, model_(load_model(data_path() / "FlightHelmet" / "FlightHelmet.gltf", graphics_))
	{
		...
		material_.normals.uniform = graphics_.create_uniform("material.normals", uniform_type::texture);
		material_.roughness.uniform = graphics_.create_uniform("material.roughness", uniform_type::texture);
		...
		material_.normals.texture = graphics_.create_texture(load(data_path() / "crate_specular.png"));
	...
	void draw(const game_time delta_time) override
	{
		...
		auto light_pos = glm::vec3(0.0f, 0.5f, -0.5f);
		...
		glm::vec4 ambient = color::light_grey();
		glm::vec4 red = color::light_sky_blue();
		glm::vec4 blue = color::pale_green();
		glm::vec4 color = glm::mix(red, blue, sin_val);

		for (const auto& mesh : model_.meshes)
		{
			glm::mat4 translate = glm::translate(glm::mat4(), mesh.trans.get_position());
			glm::mat4 rotate = glm::mat4_cast(mesh.trans.get_rotation());
			glm::mat4 scale = glm::scale(glm::mat4(), mesh.trans.get_scale());

			auto blend = mesh.alpha;

			model = translate * rotate * scale;

			graphics_.begin()
				.set_vertex_buffer(mesh.vertex_buffer, 0, mesh.vertex_count)
				.set_index_buffer(mesh.index_buffer, mesh.index_count)
				.set_program(cube_program_)
				.set_texture(0, material_.albedo.uniform, mesh.mat.albedo.handle)
				.set_texture(1, material_.normals.uniform, mesh.mat.normals.handle)
				.set_texture(2, material_.roughness.uniform, mesh.mat.roughness.handle)
