...
namespace moka
{
	...
	void run_native_event_loop(graphics_device& sender)
	{
		SDL_Event e;
	...
	program_handle create_shader(graphics_device& sender)
	{
	...
	{
		run_native_event_loop(graphics);

		const vector3 vector(0, 0, 0);
		const matrix2 matrix(0, 0, 0, 0);
...
int main()
{
	...
	std::thread t([&]()
	{
		...
		window.make_current(worker_context);
